...
export default function MarketStats() {
  ...
  return (
    <div className="bg-black text-white p-6 rounded-xl max-w-5xl mx-auto">
      ...
      {/* High / Low Bar */}
      <div className="mt-4">
        <p className="text-white opacity-80">High / Low Price (24h)</p>
        <div className="relative h-2 bg-gray-700 rounded mt-2">
          <div
            className="absolute top-0 h-2 bg-blue-500 rounded"
            style={{ width: `${position}%` }}
          ></div>
        </div>
        <div className="flex justify-between text-sm text-white opacity-80 mt-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
        <div className="bg-gray-900 p-4 rounded-xl">
          <p className="text-white opacity-80">Market Cap</p>
          <p className="text-xl font-semibold">
            ${data.market_data.market_cap.usd.toLocaleString()}
          </p>
        </div>
        <div className="bg-gray-900 p-4 rounded-xl">
          <p className="text-white opacity-80">Fully Diluted</p>
          <p className="text-xl font-semibold">
            ${data.market_data.fully_diluted_valuation.usd.toLocaleString()}
          </p>
        </div>
        <div className="bg-gray-900 p-4 rounded-xl">
          <p className="text-white opacity-80">24h Volume</p>
          <p className="text-xl font-semibold">
            ${data.market_data.total_volume.usd.toLocaleString()}
          </p>
        </div>
        <div className="bg-gray-900 p-4 rounded-xl">
          <p className="text-white opacity-80">Circulating Supply</p>
          <p className="text-xl font-semibold">
